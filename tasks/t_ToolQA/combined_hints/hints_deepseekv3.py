...
Hint: When solving the question by exploring the tabular database (airbnb, yelp, flights and coffee), remember to learn about the database by viewing the column names. Do not try to solve the task in one step. Break it down into smaller steps, and remember to print the result of each step.Hint: When solving questions regarding Yelp database, note that you should not filter the data by the column "categories" using `data_filter()`.

Hint: When solving questions regarding Yelp database, note that you should not filter the data by the column "categories" using `data_filter()`.
...
Question: What are the coordinates of Perenn in Bay in area with postal code 23444, Reno, MO?
Hint:
Filter the data to get rows where 'name' is 'Perenn in Bay', 'postal_code' is '23444, 'city' is 'Reno' and 'state' is 'MO'.
...
Do not use `data_filter()` for this step. Instead, strictly follow the code below:
...
4: Return the title(s) of the paper(s). The answer should be exactly the same as what in the database.
...
Question: Which co-author of Laddha has the highest total citations for all their co-authored papers?
Hint: Remeber that if multiple co-authors have the same highest total citations, return all of them.
...
Question: Can you recommend me a hotel room with the lowest price in Bushwick?
Solution:
...
Question: When did Paul attend Movie Party on 2022/01/01?
Start by retrieving, e.g., 10 documents. 

Question: Where did Movie Party that Paul attended take place on 2022/01/01?
Start by retrieving, e.g., 10 documents. 
When answering questions about the location of an event, provide the exact name of the place as stated in the document, without any context. For example, if the document indicates that the event took place at 'the garden of The Central Park', the answer should be 'the garden of The Central Park' but not 'Central Park'.
...
Question: When should I schedule a meeting with Paul from 9:00 AM to 6:00 PM on 2023/01/01? / What events does Paul have on 2023/01/01?
Solution:
1. For these two questions, start by retrieving large number of documents related to Paul: `retrieved_docs = retrieve_agenda('Paul', 1000)`.
...
5. For question about the schedule, seperate different time slots with a comma, e.g., '10:00 AM-12:00 PM, 2:00 PM-4:00 PM'.
For question about all events of Paul, read the retrieved documents to find out all events Paul has on this day. Provide the name of all events in a comma-separated list, e.g., 'dinner with families, reading class'.
...
Question: What was the highest coffee price '2010-01-15' to '2010-05-05'?
Hint: Get the values from the column "High".
...
Solution:
1. Filter the data to get the rows from '2010-01-15' to '2010-05-05'.
2. Get the value of 'Close' for each day into `close_prices`.
3. Convert strings to lists:
`close_prices_list = [float(price) for price in close_prices.split(', ')]`
...
Question: What was the difference between the highest and the lowest coffee price from 2010-01-15 to2010-05-05?
Hint:
1. Filter the data to get the rows from '2010-01-15' to '2010-05-05'.
...
Question: 
What was the local arrival time of the AB123 flight from CDC to EFE on 2020-01-01? / What was the CRS-recorded arrival time of the AB123 flight from CDC to EFE on 2020-01-01? / What was the departure time of the AB123 flight from CDC to EFE on 2020-01-01?
Hint:
1. Filter the data based on the question.
2. Get the value of 'ArrTime' / 'CRSArrTime' / 'DepTime' for the row into a variable `arr_time` / `crs_arr_time` / `dep_time`.
3. You must view the value by `print(arr_time)` / `print(crs_arr_time)` / `print(dep_time)`.
...
Hint: Get the value of 'AirTime' and convert it to an integer as the answer.
...
3. View the value by `print(taxi_in)`.
...
Solution:
1. Filter the data to get rows where 'Origin' is 'CDC', 'Dest' is 'EFE' and 'Airline' is 'Air Lines Inc.'.
...
2. Keep only filtered rows with 'categories' containing 'Food'.
In this step, do not use the tool. Instead, use the following code:
`data_food = data[data['categories'].str.contains('Food')]`.
...
`num = [1 for rate in rates.split(', ') if float(rate)>=4]`.
""",
# general hints
