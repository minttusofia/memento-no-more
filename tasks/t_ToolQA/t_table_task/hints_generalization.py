...
Directly write the answer in the correct format. Do not programmatically convert the value.
"""
...
`air_times_list = [float(time) for time in air_times.split(', ')]`
4. There might be nan values in 'AirTime'. When the calculation involves air time, you should remove the 'nan' values by ensuring the air time is greater than or equal to 0.
And for this question, remove both the 'nan' values from the 'air_times_list', and corresponding values in 'distances_list':
...
"""
hint_flights_distance_str = """
Hint: 'Distance' is stored in the database as string. Do not use `data_filter()` to filter by distance. Instead, when you need the value of 'Distance', use `get_value()` and convert it to a list of floating-point numbers.
"""
# yelp
...
Do not use the `data_filter()` for this step. Instead, always remember to use the following codes:
...
3. In the airbnb database, 'price' is in a special format. We need to process values of it before calculating.
Convert `prices` to a list of floating-point numbers. Do it strcitly as follows: 
...
2. Get the value of 'price' for filtered rows into a variable `prices`.
3. In the airbnb database, 'price' is in a special format. We need to process values of it before calculating.
Convert `prices` to a list of floating-point numbers. Do it strcitly as follows: Do it strcitly as follows: 
```prices = prices.replace('$','').replace(',','')
prices_list = [float(price) for price in prices.split()]```
...
lats_list = [float(lat) for lat in lats.split(', ')]
lons_list = [float(lon) for lon in lons.split(', ')]
prices = prices.replace('$','').replace(',','') # In the airbnb database, 'price' is in a special format. We need to process values of it before calculating.
prices_list = [float(price) for price in prices.split()]```
