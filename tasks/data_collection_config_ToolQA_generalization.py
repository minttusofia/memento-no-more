# Test the agent on unseen questions / Test the agent using ToolQA + RA hints
...
from tasks.t_ToolQA.combined_hints.hints_seen_questions import HINTS
from tasks.t_ToolQA.combined_hints.hints_seen_questions_deepseekv3 import HINTS as HINTS_DEEPSEEKV3
from tasks.t_ToolQA.combined_hints.hints_seen_questions_gpt4o import HINTS as HINTS_GPT4o


def graph_task_generalization(level: str = 'train-seen-questions', all_tools: bool = True, question_index: int = 0, number_of_questions: int = 100, use_generic_task_description: bool = True, n_trials: int = 1) -> TaskSet:
    """
    level: ['train', 'val', 'train-seen-questions', 'val-seen-questions'],
    ...
    from tasks.t_ToolQA.t_graph_task.t_graph_task import (
        ...
        hint_institution,
        hint_duplicate_citations,
    )
    import json
    from tasks.t_ToolQA.t_graph_task.utils import DATA_BASE_DIR
    ...
    question_answer = read_qa(DATA_BASE_DIR // f'{level}/dblp-{level}.jsonl') # (questions, answers, types)
    ...
    for task in tasks:
        ...
        if task.task_question_type == 'how many papers': # How many papers in the DBLP citation network cited
            guidelines.append(hint_citation)
        elif task.task_question_type == 'papers write together': # How many papers did Xianhao Jin and Francisco Servant write together
            guidelines.append(hint_collaborations)
        elif task.task_question_type == 'number of papers written by A' or task.task_question_type == 'papers written by A': # What papers did Rama Syamala Sreepada write / How many papers did Yuheng Deng write
            guidelines.append(hint_paper)
        elif task.task_question_type == 'most cited paper': # Which is/are the most cited paper(s) written by
            guidelines.append(hint_paper)
            guidelines.append(hint_most_cited)
            guidelines.append(hint_duplicate_citations)
        elif task.task_question_type == 'co-author with the highest citation': # Which co-author of Moustafa H. Aly has the highest total citations
            guidelines.append(hint_paper)
            guidelines.append(hint_duplicate_citations)
        elif task.task_question_type == 'accumulated citations': # How many accumulated citations
            guidelines.append(hint_collaborations)
            guidelines.append(hint_duplicate_citations)
        elif task.task_question_type == 'papers by collaborators and others': # "What is the total number of papers authored by
            guidelines.append(hint_all_collaborations)
        elif task.task_question_type == 'collaborate most': # Who collaborated with Jiaguo Liu most
            guidelines.append(hint_paper)
        elif task.task_question_type == 'institution': # What institutions participated in
...
def graph_task_test_generalization(level: str = 'test-unseen-questions', tools: str = 'none', question_index: int = 0, number_of_questions: int = 100, use_combined_guidelines: bool = True, combined_hints_model: str = 'llama3.1-70b', n_trials: int = 1) -> TaskSet:
    """
    Use this function to test models on the graph task

    level: ['train', 'test', 'train-seen-questions', 'test-unseen-questions']
    tools: ['none', 'relevant', 'all', 'all-long']
    ...
    if tools == 'none':
        ...
        tool_docs_list = ["load_graph", "check_nodes", "check_neighbours", "check_edges", "complete_task",] if tools == 'relevant' else ["load_db", "data_filter", "get_value", "load_graph", "check_nodes", "check_neighbours", "check_edges", "retrieve_agenda", "complete_task",]
...
def table_task_generalization(db_variant: str = 'flights', level: str = 'train-seen-questions', all_tools: bool = True, question_index: int = 0, number_of_questions: int = 100, format_tips: bool = True, custom_init_script: bool = False, basic_normalization: bool = True, use_generic_task_description: bool = True, n_trials: int = 1) -> TaskSet:
    """
    ...
    from tasks.t_ToolQA.t_table_task.t_table_task import (
        TableTask, step_by_step_tips, answer_tip,
 )
    from tasks.t_ToolQA.t_table_task.hints_generalization import (
        hint_coffee_print, hint_coffee_price_change, hint_coffee_highest_increase, hint_coffee_increase_times, hint_coffee_percentage_increase, hint_coffee_percentage_change, hint_coffee_higher_close_price, hint_coffee_price_range, hint_coffee_bullish_or_bearish,
        hint_flights_deptime, hint_flights_difftime, hint_flights_number, hint_flights_extramin, hint_flights_arrtime, hint_flights_crstime, hint_flights_airtime, hint_flights_distance, hint_flights_unique, hint_flights_avgtime, hint_flights_fastest, hint_flights_totalnumber, hint_flights_avgspeed, hint_round, hint_flights_taxin, hint_flights_diverted, hint_flights_avgairtime, hint_flights_cancelled, hint_flights_distance_str,
    ...
    for task in tasks:
        ...
        if db_variant == 'flights':
            ...
            if task.task_question_type == 'flights with long distance':
                guidelines.append(hint_flights_distance_str)
...
def table_task_test_generalization(db_variant: str = 'flights', level: str = 'easy', question_index: int = 0, number_of_questions: int = 100, tools: str = 'none', use_combined_guidelines: bool=True, combined_hints_model: str = 'llama3.1-70b', n_trials: int = 1) -> TaskSet:
    """
    Use this function to test our agent / baselines.
    ...
    if tools == 'none':
        ...
        tool_docs_list = ["load_db", "data_filter", "get_value", "complete_task",] if tools == 'relevant' else ["load_db", "data_filter", "get_value", "load_graph", "check_nodes", "check_neighbours", "check_edges", "retrieve_agenda", "complete_task",]
...
def text_task_generalization(level: str = 'train-seen-questions', all_tools: bool = True, question_index: int = 0, number_of_questions: int = 100, use_generic_task_description: bool = True, n_trials: int = 1) -> TaskSet:
    """
    level: ['train', 'val', 'train-seen-questions', 'val-seen-questions']
    ...
    for task in tasks:
        ...
        if task.task_question_type == 'schedule':
            guidelines.append(hint_schedule_generalization)
...
def text_task_test_generalization(level: str = 'test', tools: str = 'none', question_index: int = 0, number_of_questions: int = 100, use_combined_guidelines: bool = True, combined_hints_model: str = 'llama3.1-70b', n_trials: int = 1) -> TaskSet:
    """
    level: ['train', 'test', 'train-seen-questions', 'test-unseen-questions']
    tools: ['none', 'relevant', 'all', 'all-long']
            'none': none of tools' names/documentation is provided;
    ...
    if tools == 'none':
        ...
        tool_docs_list = ["retrieve_agenda", "complete_task",] if tools == 'relevant' else ["load_db", "data_filter", "get_value", "load_graph", "check_nodes", "check_neighbours", "check_edges", "retrieve_agenda", "complete_task",]
    ...
    taskset_name = f"text_task_{level}_{tools}_tools{'+combined_guidelines' if use_combined_guidelines else ''}"
